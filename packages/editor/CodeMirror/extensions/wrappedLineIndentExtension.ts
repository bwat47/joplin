...
import { RangeSetBuilder, StateEffect, countColumn, type EditorState, type Line, type Text, type Transaction } from '@codemirror/state';
...
const wrappedLineClass = 'cm-wrapped-line-indent';
const taskListCheckboxPattern = /\[[ xX]\]/;
const listPrefixPattern = /^([ \t]*(?:[-*+]|\d+[.)])[ \t]+(?:\[[ xX]\][ \t]+)?)/;
...
export const getTabReplacementWidth = (textBeforeTab: string, tabSize: number, characterWidth: number): number => {
	const column = countColumn(textBeforeTab, tabSize);
	return (tabSize - (column % tabSize)) * characterWidth;
};

const parseListPrefix = (lineText: string): string | null => {
	const text = listPrefixPattern.exec(lineText)?.[1];
	if (!text) {
		return null;
	}
...
export const parseIndentPrefix = (lineText: string): ParsedIndentPrefix | null => {
	...
	if (blockquoteMatch?.[1]) {
		...
		if (listPrefix) {
			return { text: quotePrefix + listPrefix };
		}

		return { text: quotePrefix };
	}

	const listPrefix = parseListPrefix(lineText);
	if (listPrefix) {
		return { text: listPrefix };
	}

	const whitespaceMatch = /^([ \t]+)/.exec(lineText);
	if (whitespaceMatch?.[1]) {
		return { text: whitespaceMatch[1] };
	}

	return null;
};

const intersectsFoldedRange = (state: EditorState, from: number, to: number): boolean => {
...
export const isBlockCodeNode = (nodeName: string): boolean => {
	return /^(?:CodeBlock|FencedCode|CodeInfo)$/i.test(nodeName);
};

export const isHorizontalRuleNode = (nodeName: string): boolean => {
	return /^HorizontalRule$/i.test(nodeName);
};

const isIndentExcludedNode = (nodeName: string): boolean => {
	return isBlockCodeNode(nodeName) || isHorizontalRuleNode(nodeName);
};

const isInIndentExcludedSyntax = (state: EditorState, from: number, to: number): boolean => {
...
const addTabReplacementDecorations = (
...
const estimatePrefixWidth = (prefix: string, view: EditorView, spaceCharacterWidth: number | null = null): number => {
	const characterWidth = view.defaultCharacterWidth / view.scaleX;
	let width = 0;
	...
	for (let index = 0; index < prefix.length; index++) {
		...
		if (character === '\t') {
			width += getTabReplacementWidth(prefix.slice(0, index), view.state.tabSize, characterWidth);
		} else if (character === ' ') {
			width += spaceCharacterWidth ?? characterWidth;
		} else {
...
const measureSpaceCharacterWidth = (view: EditorView, target: MeasurementTarget): number | null => {
...
export const getLineDecorationStyle = (width: number, linePaddingLeft: number): string => {
	const paddingLeft = width + linePaddingLeft;
	return `padding-left: ${paddingLeft}px; text-indent: -${width}px;`;
};

const getLinePaddingLeft = (view: EditorView, fallbackPaddingLeft: number): number => {
	const lineElement = view.contentDOM.querySelector<HTMLElement>(`.cm-line:not(.${wrappedLineClass})`);
	if (!lineElement) {
...
const createLineDecoration = (width: number, linePaddingLeft: number): Decoration => {
	return Decoration.line({
		attributes: {
			class: wrappedLineClass,
			style: getLineDecorationStyle(width, linePaddingLeft),
		},
	});
};

const getLineMeasurementKey = (line: Line, prefix: ParsedIndentPrefix): string => {
	return `${line.from}:${prefix.text}`;
};

const getFallbackPrefixKey = (prefixText: string): string => {
	return prefixText.replace(taskListCheckboxPattern, '[ ]');
};

const getMeasurementSignature = (view: EditorView): string => {
	return [view.defaultCharacterWidth, view.defaultLineHeight, view.scaleX, view.scaleY, view.state.tabSize].join(':');
};

export const isFullDocumentReplace = (transaction: Transaction): boolean => {
...
class WrappedLineIndentPlugin implements PluginValue {
	...
	public update(update: ViewUpdate): void {
		...
		const receivedMeasurementUpdate = update.transactions.some(transaction => {
			return transaction.effects.some(effect => effect.is(measurementsChanged));
		});
		...
		const externalRefreshTrigger = measurementsNeedRefresh || editorContentOrStateChanged || viewportOrGeometryChanged;
		...
		if (shouldRebuildDecorations) {
			...
			this.decorations = this.buildDecorations({
				forceVisibleLineMeasurements: fullDocumentReplaced || measurementsNeedRefresh || update.selectionSet || update.focusChanged || viewportOrGeometryChanged,
			});
			this.scheduleMeasure();
		}
	}
	...
	private addDecorationsForLine(builder: RangeSetBuilder<Decoration>, line: Line, options: BuildDecorationsOptions): string | null {
		...
		if (intersectsFoldedRange(this.view.state, line.from, prefixTo) || isInIndentExcludedSyntax(this.view.state, line.from, prefixTo)) {
			return null;
		}
	...
	private scheduleMeasure(): void {
		...
		this.view.requestMeasure<MeasureReadResult>({
			...
			write: (result) => {
				...
				const shouldRefreshForIncompleteMeasurement = result.needsRetry && !this.incompleteMeasurementRefreshSpent;
	...
	private scheduleDecorationsRefresh(): void {
		if (this.destroyed || this.refreshFrame !== null) {
			return;
		}
	...
	private measurePrefixes(view: EditorView, targets: Map<string, MeasurementTarget>, measuredDoc: Text, fallbackPaddingLeft: number): MeasureReadResult {
...
export default wrappedLineIndentExtension;
